        ...
        using (var udp = new UdpClient(AddressFamily.InterNetwork))
        {
            ...
            using (var cts = new CancellationTokenSource(TimeSpan.FromMinutes(Duration)))
            {
                while (!cts.IsCancellationRequested)
                {
                    try
                    {
                        // In .NET Framework there is no ReceiveAsync(..., cancellationToken) or Task.WaitAsync.
                        // Use a WhenAny between ReceiveAsync and a canceled Task.Delay to support cancellation.
                        ...
                        try
                        {
                            ...
                            int offset = 12;
                            string name = DnsParser.ReadNameSimple(buf, ref offset);
                            ushort txId = DnsParser.ReadUInt16(buf, 0);
                            ushort ancount = DnsParser.ReadUInt16(buf, 6);
                            // only respond to Questions to avoid Answer loop
                            bool isQuestion = ancount == 0;

                            if (name.IndexOf("wpad", StringComparison.OrdinalIgnoreCase) >= 0 && isQuestion)
                            {
                                byte[] response = DnsParser.BuildMdnsAResponse(name, AdvertisedAddress, txId);
                                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] mDNS WPAD Traffic found from {remote.Address}! Sending Response...");

                                var sent = await udp.SendAsync(response, response.Length, remote).ConfigureAwait(false);
                            }
                        }
                        catch (Exception ex)
                        {
                ...
                //close down socket and drop from multicast
                udp.DropMulticastGroup(multicastAddress);
                udp.Dispose();
...
public static class DnsParser
{
    ...
    public static string ReadNameSimple(byte[] msg, ref int offset)
    {
        ...
        while (true)
        {
            ...
            // Read label and decode as UTF-8
